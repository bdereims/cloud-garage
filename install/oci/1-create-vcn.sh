...
COMP_ID=$( oci iam compartment create -c ${COMP_ID_ROOT} --name ${COMP_NAME} --description "automaticaly created compartment" 2> /dev/null | jq '.data | .id' | sed -e "s/\"//g" )
#COMP_ID=$( oci iam compartment list --compartment-id-in-subtree true --all | jq '.["data"] | .[] | select(."name" == "'${COMP_NAME}'") | .id' | sed -e "s/\"//g" )

# create vcn
oci network vcn create --compartment-id ${COMP_ID} --display-name "${VCN_NAME}" --dns-label ${VCN_NAME} --cidr-block "${VCN_CIDR}"
